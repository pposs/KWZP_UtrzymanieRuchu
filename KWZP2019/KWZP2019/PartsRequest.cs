...
namespace KWZP2019
{
    public partial class PartsRequest : Form
    {
        ...
        public void ReloadData()
        {
            ...
            if (showHistory)
            {
                dataPartRequestView.DataSource = db.vPartsRequestViews
                    .Where(status => status.StatusPart >= 2).ToList();
            }
        ...
        private void pbDateBefore_Click(object sender, EventArgs e)
        {
            ...
            {
                dataPartRequestView.DataSource = db.vPartsRequestViews
                      .Where(partreq => partreq.RequestDate < dpsRequestDateFrom.Value)
                      .Where(status => status.StatusPart >= 2).OrderBy(partreq => partreq.RequestDate).ToList();
            }
        ...
        private void pbDateRange_Click(object sender, EventArgs e)
        {
            if (showHistory)
            {
                dataPartRequestView.DataSource = db.vPartsRequestViews
                      .Where(partreq => partreq.RequestDate >= dpsRequestDateFrom.Value &&
                      partreq.RequestDate <= dpsRequestDateTo.Value).Where(status => status.StatusPart >= 2)
                      .OrderBy(partreq => partreq.RequestDate).ToList();
            }
        ...
        private void pbDateEqual_Click(object sender, EventArgs e)
        {
            ...
            {
                dataPartRequestView.DataSource = db.vPartsRequestViews
                      .Where(partreq => partreq.RequestDate.Day == dpsRequestDateFrom.Value.Day &&
                      partreq.RequestDate.Month == dpsRequestDateFrom.Value.Month &&
                      partreq.RequestDate.Year == dpsRequestDateFrom.Value.Year)
                      .Where(status => status.StatusPart >= 2).OrderBy(partreq => partreq.RequestDate).ToList();
            }
        ...
        private void pbDateAfter_Click(object sender, EventArgs e)
        {
            ...
            {
                dataPartRequestView.DataSource = db.vPartsRequestViews
                      .Where(partreq => partreq.RequestDate > afterDate)
                      .Where(status => status.StatusPart >= 2).OrderBy(partreq => partreq.RequestDate).ToList();
            }
        ...
        private void btnRequestPart_Search_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    dataPartRequestView.DataSource = db.vPartsRequestViews
                    .Where(ps => ps.PartName.Contains(tbRequestSearch.Text))
                    .Where(status => status.StatusPart >= 2).ToList();
                }
            ...
            try
            {
                if (selectedRow)
                {
                        ...
                        MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                    {
                        ...
                        PartRequest requestdel = new PartRequest();
                        requestdel = db.PartRequests.Where(id => id.IdPartRequest == selectedID).First();

                        db.PartRequests.Remove(requestdel);
            ...
            if (selectedRow)
            {
                ...
                else
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        partrequest = db.PartRequests.Where(id => id.IdPartRequest == selectedID).First();
                        partrequest.StatusPart = 2;

                        Part part = new Part();
                        part = db.Parts.Where(name => name.PartName == selectedName).First();
